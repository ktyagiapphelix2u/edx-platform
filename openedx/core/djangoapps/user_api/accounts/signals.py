...
def redact_social_auth_pii_before_deletion(sender, instance, **kwargs):  # pylint: disable=unused-argument
    ...
    or any other method), PII is redacted first. This prevents PII from persisting in downstream
    systems that may retain soft-deleted records.

    Note: We call redact_user_social_auth_pii which saves the redacted data before the actual
    deletion happens. This ensures that any database replication or sync processes will capture
    the redacted state before the record is marked as deleted.
    """
    try:
